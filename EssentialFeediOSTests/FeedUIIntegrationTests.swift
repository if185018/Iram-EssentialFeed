...
final class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_feedImageViewRetryButton_is_VisibleOnInvalidImageData() {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_doesNotRenderLoadedImagesWhenNotVisibleAnymore() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(with: [makeImage()])
        sut.simulateFeedImageViewVisible(at: 0)
    ...
    func test_loadImageDataCompletion_dispacthesFromBackgroundToMainThread() {
        let (sut, loader) = makeSUT()
        ...
        let exp = expectation(description: "Wait for background queeue work")
        DispatchQueue.global().async {
